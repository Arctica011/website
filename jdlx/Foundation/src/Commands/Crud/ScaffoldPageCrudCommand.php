...
class ScaffoldPageCrudCommand extends Command
{
    use ModelUtil;
     ...
     * @var string
     */
    protected $name = 'crud:scaffold:page';
     ...
     * @var string
     */
    protected $description = 'Creates a new page for a given model and page';
    ...
    public function handle()
    {
        ...
        if(in_array("all", $types)){
            $types = ModelGenerator::PAGE_TYPES;
        }

        foreach ($types as $type){
            if(!in_array($type, ModelGenerator::PAGE_TYPES)){
                $this->warn("Invalid type {$type}, not generated");
                continue;
            }
            $dest = $generator->generateFrontEndPage($type, $namespace, $path, $this->option("force"));
            $dest = Path::toRelative(getcwd(), $dest);
            $this->info("${type}Page generated at [{$dest}]");
    ...
    {
        $types = implode("|", [...ModelGenerator::PAGE_TYPES, "all"]);
        return [
    ...
    {
        return $this->option("path") ?? "dashboard";
    }
}
